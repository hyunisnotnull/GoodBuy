...
const chatModel = {
    ...
    findChatRooms: (U_ID, callback) => {
        ...
        const sql = `
            SELECT C.*, 
                   U.U_ID AS otherId,
                   U.U_NICK AS otherNick,
                   U.U_PROFILE_THUM AS otherthum
            FROM TBL_CHAT C
            JOIN TBL_USER U
            ON U.U_ID = CASE 
                           WHEN C.CH_SENDER_ID = ? THEN C.CH_RECEIVER_ID
                           ELSE C.CH_SENDER_ID
                       END
            WHERE (C.CH_SENDER_ID = ? AND C.CH_ACTIVE IN (1, 2))
               OR (C.CH_RECEIVER_ID = ? AND C.CH_ACTIVE IN (1, 3))
        `;
        DB.query(sql, [U_ID, U_ID, U_ID], callback);
    ...
    createMessage: (messageData, callback) => {
        ...
        const sql = `
            INSERT INTO TBL_MESSAGE (M_CHAT_CH_NO, M_SENDER_ID, M_SENDER_NICK, 
            M_RECEIVER_ID, M_RECEIVER_NICK, M_CONTENT)
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        const params = [
            messageData.roomId,
            messageData.senderId,
            messageData.senderNick,
            messageData.receiverId,
            messageData.receiverNick,
            messageData.content
        ];
    ...
    saveChatImage: (messageId, imagePath, callback) => {
        ...
        const sql = `INSERT INTO TBL_CHAT_IMAGE (CI_MESSAGE_M_NO, CI_FILE)
                     VALUES (?, ?)`;
