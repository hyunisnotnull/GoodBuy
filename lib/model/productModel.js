...
const productModel = {
    ...
    getProductById: (productId, u_no, callback) => {
        const sql = `SELECT P.*, PI_FILE P_IMAGE, PI_NO, U_CLASS, U_ACTIVE, W_NO 
                     FROM TBL_PRODUCT P 
    ...
    getAuctionProducts: (userId, p_category, sortBy, offset, limit, callback) => {
        ...
        if (sortBy === 'price_low') {
            ...
            sql += ' ORDER BY P_PRICE DESC';
        } else if (sortBy === 'imminent') {
            sql += ' ORDER BY P_TRADE_DATE';
        } else {
            sql += ' ORDER BY P_MOD_DATE DESC';
        } 
    ...
    getProductsByOwnerWithCategoryAndState: (ownerId, p_category, p_state, offset, limit, callback) => {
        const sql = `
            SELECT P.*, I.PI_FILE AS P_IMAGE, AU.*
            FROM TBL_PRODUCT P
            JOIN TBL_PRODUCT_IMAGE I ON P.P_IMAGE_NO = I.PI_NO
            LEFT JOIN TBL_AUCTION AU ON AU.AU_PRODUCT_NO = P.P_NO
            WHERE P.P_OWNER_ID = ? 
            AND P.P_STATE > 0 
            ${p_category !== '0' ? 'AND P.P_CATEGORY = ?' : ''} 
            ${p_state !== '0' ? 'AND P.P_STATE = ?' : ''} 
            ORDER BY P.P_MOD_DATE DESC
            LIMIT ? OFFSET ?;
            `;
