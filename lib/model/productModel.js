...
const productModel = {
    ...
    getProductById: (productId, callback) => {
        const sql = 'SELECT P.*, PI_FILE P_IMAGE, PI_NO, U_CLASS FROM TBL_PRODUCT P JOIN TBL_PRODUCT_IMAGE ON P_NO = PI_P_NO JOIN TBL_USER ON P_OWNER_ID = U_ID WHERE P_NO = ?';
    ...
    getProductsForAuctionCount: (userId, callback) => {
        let sql = `
            SELECT COUNT(*) AS count
            FROM TBL_AUCTION A
    ...
    getAuctionProducts: (userId, offset, limit, callback) => {
        let sql = `
        ...
        sql += ' ORDER BY P.P_NO DESC LIMIT ? OFFSET ?';
        state.push(limit, offset);
