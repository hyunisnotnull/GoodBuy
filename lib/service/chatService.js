...
const chatService = {
    ...
    getChatRooms: (req, res) => {
        ...
        const query = `SELECT * FROM TBL_CHAT WHERE CH_OWNER_ID = ? OR CH_OTHER_ID = ?`;
        DB.query(
            query, 
            [userId, userId], 
            (err, results) => {
    ...
    saveChatMessage: (senderId, senderNick, receiverId, receiverNick, message, roomId) => {
        const query = `INSERT INTO TBL_CHAT (CH_OWNER_ID, CH_OWNER_NICK, CH_OTHER_ID, CH_OTHER_NICK, CH_BODY, CH_ACTIVE) 
                        VALUES (?, ?, ?, ?, ?, 2)`;
