    ...
    listMyProductForm: (req, res) => {
        productModel.getAllCategories((error, category) => {
            ...
            productModel.getAllStates((error, result) => {
                ...
                // 전체 상품 수 가져오기
                productModel.getProductsCountByOwner(req.user.U_ID, (error, totalCount) => {
                    ...
                    // 전체 상품 가져오기
                    productModel.getProductsByOwner(req.user.U_ID, offset, itemsPerPage, (error, allProducts) => {
                        if (error) {
                            console.error(error);
                            return res.render('product/list_my_product_form', { loginedUser: req.user, events: [], pagination: {} });
                        }
