...
const { removeHtmlTagsAndEntities, cleanImageUrl } = require('../config/utils');
        ...
        productModel.getMonthlyRegisteredPricesByKeyword(keyword, (error, registeredPrices) => {
            ...
            productModel.getMonthlySalePricesByKeyword(keyword, (error, salePrices) => {
                ...
                const averageRegisteredPrice = registeredPrices.length > 0 ? 
                    registeredPrices.reduce((sum, price) => sum + parseFloat(price.averagePrice), 0) / registeredPrices.length : 0;
    
                const averageSalePrice = salePrices.length > 0 ? 
                    salePrices.reduce((sum, price) => sum + parseFloat(price.averagePrice), 0) / salePrices.length : 0;
    
                console.log('searchType', searchType);
                console.log('averageRegisteredPrice', averageRegisteredPrice);
                console.log('averageSalePrice', averageSalePrice);
