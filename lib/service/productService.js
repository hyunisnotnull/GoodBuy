...
const productService = {
    ...
    listMyProductForm: (req, res) => {
        const selectedCategory = req.query.category || '0'; // 기본값은 전체
        const selectedState = req.query.state || '0'; // 기본값은 전체
        ...
        productModel.getAllCategories((error, category) => {
            ...
            productModel.getAllStates((error, result) => {
                ...
                // 전체 상품 수 가져오기
                productModel.getProductsCountByOwnerWithCategory(req.user.U_ID, selectedCategory, (error, totalCount) => {
                    if (error || totalCount === undefined || totalCount === null) {
                        console.error(error);
                        return res.render('product/list_my_product_form', { loginedUser: req.user, events: [], pagination: {} });
                    }
                    ...
                    // 전체 상품 가져오기
                    productModel.getProductsByOwnerWithCategory(req.user.U_ID, selectedCategory, offset, itemsPerPage, (error, allProducts) => {
                        if (error) {
                            console.error(error);
                            return res.render('product/list_my_product_form', { loginedUser: req.user, events: [], pagination: {} });
                        }
