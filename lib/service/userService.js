...
const userService = {
    ...
    cancelWishlist: (req, res) => {
        ...
        userModel.cancelWishlist(req.user.U_NO, productNo, (error, results) => {
            ...
            // 찜 목록 갱신된 결과를 전달
            userModel.getWishlist(req.user.U_NO, (err, updatedWishlist) => {
                if (err) {
                    console.log(err);
                    return res.status(500).send({ success: false, message: '찜 목록을 불러오는 데 오류가 발생했습니다.' });
                }
    
                // 성공적인 응답
                res.send({ success: true, message: '찜 취소가 완료되었습니다.', updatedWishlist: updatedWishlist });
            });
        });
    },
