            ...
            (error, result) => {
                if (error) {
                    ...
                    if (result[0].CNT > 0) {
                        ...
                        sql = `
                            INSERT INTO TBL_USER(U_ID, U_PW, U_MAIL, U_PHONE ${req.file !== undefined ? `, U_PROFILE_THUM` : ``}) 
                            VALUES(?, ?, ?, ? ${req.file !== undefined ? `, ?`: ``})
                        `;
                        state = [post.u_id, bcrypt.hashSync(post.u_pw, 10), post.u_mail, post.u_phone];
    ...
    modifyConfirm: (req, res) => {
        ...
        `UPDATE TBL_USER SET 
            ...
            WHERE 
                U_NO = ?
        `;
        let state = [post.u_mail, post.u_phone];
