...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        // 채팅방 입장 이벤트
        socket.on('joinRoom', ({ roomId, senderId, senderNick, otherId, otherNick, otherthum, lastExitTime }) => {
            socket.join(roomId);
            console.log(`사용자 ${senderId}가 방 ${roomId}에 입장했습니다.`);

            // 이전 채팅 기록을 불러와 클라이언트로 전송
            chatService.getChatHistory(roomId, senderId, senderNick, otherId, otherNick, lastExitTime)
                .then((chatHistory) => {
