...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        console.log(`유저 사용자 연결됨: ${socket.id}`);

        // 채팅방 입장 이벤트
        socket.on('joinRoom', ({ roomId, senderId, senderNick, otherId, otherNick, otherthum, validExitTime }) => {
            console.log("joinRoom 이벤트 수신 - roomId:", roomId, "senderId:", senderId);
            socket.join(roomId);
        ...
        socket.on('chatMessage', async ({ roomId, senderId, senderNick, otherId, otherNick, message }) => {
            console.log("chatMessage 이벤트 수신 - roomId:", roomId, "message:", message);
