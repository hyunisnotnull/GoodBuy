...
DO
BEGIN
    -- 경매 종료시간이 지난 경매 상품을 확인
    UPDATE TBL_PRODUCT
    SET P_STATE = 1 -- 취소 상태로 변경
    WHERE P_STATE = 4  -- 경매중인 상품
    AND P_TRADE_DATE < NOW()  -- 경매 종료 시간이 지난 경우
    AND NOT EXISTS (
        SELECT 1 FROM TBL_AUCTION 
        WHERE AU_PRODUCT_NO = TBL_PRODUCT.P_NO
        AND AU_STATE = 4 -- 경매중인 상품
        AND AU_BUYER_ID IS NOT NULL -- 입찰자가 있는 경우
    );

    -- 입찰자가 있으면 경매 종료 상태로 변경 (P_STATE = 2)
    UPDATE TBL_PRODUCT
    SET P_STATE = 2 -- 경매 종료 상태로 변경
    WHERE P_STATE = 4  -- 경매중인 상품
    AND P_TRADE_DATE < NOW()  -- 경매 종료 시간이 지난 경우
    AND EXISTS (
        SELECT 1 FROM TBL_AUCTION 
