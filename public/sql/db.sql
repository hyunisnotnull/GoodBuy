...
DESC TBL_PRODUCT;
SELECT * FROM TBL_PRODUCT WHERE P_CATEGORY = '7';

CREATE INDEX IDX_PRODUCT ON TBL_PRODUCT(P_NAME);
...
CREATE INDEX IDX_AUCTION ON TBL_AUCTION(AU_TRADE_DATE);
...
DROP TABLE IF EXISTS TBL_ADMIN_CHAT;
CREATE TABLE TBL_ADMIN_CHAT(
	AC_NO                INT AUTO_INCREMENT,
	AC_USER_ID           VARCHAR(20) NOT NULL,           
    AC_USER_NICK         VARCHAR(20) NOT NULL,           
    AC_TITLE             VARCHAR(50),                    -- 마지막 M_CONTENT
    AC_UNREAD_COUNT      INT DEFAULT 0,                  
    AC_ACTIVE            TINYINT DEFAULT 1,              -- 0(DELETE), 1(ACTIVE)
    AC_TIME              DATETIME DEFAULT NOW(),
    ISADMINCHAT			 INT DEFAULT 1,
	PRIMARY KEY(AC_NO)
);
...
CREATE TABLE TBL_ADMIN_MESSAGE(
	AM_NO                INT AUTO_INCREMENT,
	AM_CHAT_AC_NO        INT NOT NULL,                 -- AC_NO
	AM_SENDER_ID         VARCHAR(20) NOT NULL,         
    AM_SENDER_NICK       VARCHAR(20) NOT NULL,         
	AM_CONTENT           TEXT,                         
    AM_UNREAD_COUNT      TINYINT DEFAULT 1,            
	AM_TIME              DATETIME DEFAULT NOW(),
	PRIMARY KEY(AM_NO)
);
SELECT * FROM TBL_ADMIN_MESSAGE;

DROP TABLE IF EXISTS TBL_CHAT_ADMIN_IMAGE;
CREATE TABLE TBL_CHAT_ADMIN_IMAGE(
   AI_NO                  INT AUTO_INCREMENT,
   AI_MESSAGE_AM_NO        INT NOT NULL,        	  -- 메시지 번호 (TBL_ADMIN_MESSAGE AM_NO)
   AI_FILE                VARCHAR(100) NOT NULL,  -- 이미지 파일 경로
   PRIMARY KEY(AI_NO),
   FOREIGN KEY (AI_MESSAGE_AM_NO) REFERENCES TBL_ADMIN_MESSAGE(AM_NO) ON DELETE CASCADE
);
SELECT * FROM TBL_CHAT_ADMIN_IMAGE;
...
DROP TRIGGER IF EXISTS AFTER_INSERT_ADMIN_MESSAGE;
DELIMITER $$
CREATE TRIGGER AFTER_INSERT_ADMIN_MESSAGE
AFTER INSERT ON TBL_ADMIN_MESSAGE
FOR EACH ROW
BEGIN
    -- 새로운 메시지가 추가될 때마다 CH_UNREAD_COUNT를 증가
    UPDATE TBL_ADMIN_CHAT 
    SET AC_UNREAD_COUNT = AC_UNREAD_COUNT + 1, 
        AC_TITLE = NEW.AM_CONTENT
    WHERE AC_NO = NEW.AM_CHAT_AC_NO;
END $$
DELIMITER ;
...
FOR EACH ROW
BEGIN
IF OLD.P_STATE > 1 AND NEW.P_STATE <= 1 THEN
UPDATE TBL_USER SET U_POINT = U_POINT - 1 WHERE U_ID = NEW.P_OWNER_ID;
END IF;
END $$
DELIMITER ;
...
BEGIN
    -- 정지된 계정은 업데이트하지 않음
    IF OLD.U_CLASS = 0 THEN
        SET NEW.U_CLASS = OLD.U_CLASS;  -- U_CLASS가 0이면 그대로 두기
    ELSE
        -- 점수에 맞춰 U_CLASS를 업데이트
        IF NEW.U_POINT >= 1600 AND NEW.U_CLASS < 6 THEN
            SET NEW.U_CLASS = 6;  -- DIAMOND
        ELSEIF NEW.U_POINT >= 1100 AND NEW.U_CLASS < 5 THEN
            SET NEW.U_CLASS = 5;  -- PLATINUM
        ELSEIF NEW.U_POINT >= 700 AND NEW.U_CLASS < 4 THEN
            SET NEW.U_CLASS = 4;  -- GOLD
        ELSEIF NEW.U_POINT >= 400 AND NEW.U_CLASS < 3 THEN
            SET NEW.U_CLASS = 3;  -- SILVER
        ELSEIF NEW.U_POINT >= 200 AND NEW.U_CLASS < 2 THEN
            SET NEW.U_CLASS = 2;  -- BRONZE
        ELSEIF NEW.U_POINT >= 100 AND NEW.U_CLASS < 1 THEN
            SET NEW.U_CLASS = 1;  -- IRON
        END IF;
    END IF;
END $$
DELIMITER ;
...
BEGIN
    DECLARE RESULT INT;
    SELECT COUNT(*) INTO RESULT FROM TBL_WISHLIST WHERE W_USER_NO = NEW.W_USER_NO AND W_PRODUCT_NO = NEW.W_PRODUCT_NO;
    IF (RESULT > 0) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '이미 찜한 상품입니다.';  -- 사용자 정의 에러 메시지
    END IF;
END $$
DELIMITER ;

DROP EVENT IF EXISTS AUTO_CANCEL_AUCTION_PRODUCTS;
DELIMITER $$
CREATE EVENT IF NOT EXISTS AUTO_CANCEL_AUCTION_PRODUCTS
...
SELECT AU_PRODUCT_NO, AU_BUYER_ID FROM TBL_AUCTION WHERE AU_TRADE_DATE < NOW();
DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = 1;

OPEN CUR;
DO_LOOP: WHILE NOT DONE DO
FETCH CUR INTO NUM, ID;
IF NOT DONE THEN
IF ID IS NULL THEN
UPDATE TBL_PRODUCT SET P_STATE = 1 WHERE P_NO = NUM;
UPDATE TBL_AUCTION SET AU_STATE = 1 WHERE AU_PRODUCT_NO = NUM;
ELSE
UPDATE TBL_PRODUCT SET P_STATE = 2 WHERE P_NO = NUM;
UPDATE TBL_AUCTION SET AU_STATE = 2 WHERE AU_PRODUCT_NO = NUM;
END IF;
END IF;
END WHILE DO_LOOP;
CLOSE CUR;
...
DROP TRIGGER IF EXISTS DELETE_WISHLIST_ON_PRODUCT_STATE_CHANGE;
DELIMITER $$
CREATE TRIGGER DELETE_WISHLIST_ON_PRODUCT_STATE_CHANGE
AFTER UPDATE ON TBL_PRODUCT
FOR EACH ROW
BEGIN
    -- 상품 상태가 1(취소) 또는 0(삭제)로 변경되면 찜 목록에서 삭제
    IF NEW.P_STATE IN (0, 1) AND OLD.P_STATE NOT IN (0, 1) THEN
        DELETE FROM TBL_WISHLIST
        WHERE W_PRODUCT_NO = NEW.P_NO;
    END IF;
END $$
DELIMITER ;
